...
const FlaggedResources = () => {
    ...
    const handleResolve = async (id: number) => {
        try {
            ...
            setFlags(flags.filter((f: any) => f.id !== id));
        } catch (err) {
    ...
    return (
        <div>
            ...
            <div className="space-y-4">
                {flags.map((flag: any) => (
                    <div key={flag.id} className="p-4 border rounded-lg bg-white flex justify-between items-center">
                        <div>
                            <h3 className="font-semibold">{flag.resource.title}</h3>
                            <p className="text-sm text-gray-600">Reason: {flag.reason}</p>
                        </div>
                        <button onClick={() => handleResolve(flag.id)} className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Mark as Resolved</button>
                    </div>
                ))}
            </div>
        </div>
