...
const ResourceDetails = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        setError(
          "The requested architectural asset could not be retrieved from the nexus at this time.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleRatingClick = (rateValue: number) => {
    if (!isAuth) {
      toast.info(
        "Authentication is required to participate in project evaluations",
      );
      return;
    }
  ...
  const handleSubmitEvaluation = async () => {
    const trimmedComment = evaluationComment.trim();
    if (trimmedComment.length > 500) {
      toast.warn(
        "Evaluation critique exceeds maximum allowed length (500 tokens).",
      );
      return;
    }
    ...
    try {
      ...
      if (trimmedComment) {
        await api.post(`/resources/${id}/comments`, {
          text: `[Evaluation Feedback]: ${trimmedComment}`,
        });
      ...
      toast.success(
        `Broadcasting evaluation complete. Thank you for your review.`,
      );
      const { data } = await api.get(`/resources/${id}`);
      if (data.ratings && data.ratings.length > 0) {
        const sum = data.ratings.reduce(
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    if (!trimmedMessage) {
      toast.warn("Broadcast signal cannot be empty.");
      return;
    }
    if (trimmedMessage.length > 500) {
      toast.warn("Intelligence packet exceeds capacity (500 characters).");
      return;
    }

    setSubmittingComment(true);
    try {
      await api.post(`/resources/${id}/comments`, { text: trimmedMessage });
      toast.success(
        "Intelligence successfully synchronized with the project nexus.",
      );
      const { data: updatedResource } = await api.get(`/resources/${id}`);
      setComments(updatedResource.comments || []);
      setNewComment("");
  ...
  const handleArchive = async () => {
    toast(`Archive this asset?`, {
      description:
        "Project visibility will be restricted to administrative roles only.",
      action: {
        label: "Archive",
        onClick: async () => {
          try {
  ...
  const handleRestore = async () => {
    toast(`Restore this asset?`, {
      description:
        "Standard access permissions will be restored for all users.",
      action: {
        label: "Restore",
        onClick: async () => {
          try {
  ...
  const handleToggleVisibility = async () => {
    ...
    try {
      await api.patch(`/admin/resources/${id}/status`, {
        is_public: newIsPublic,
      });
      toast.success(
        `Visibility: ${newIsPublic ? "Global Presence" : "Internal Only"}`,
      );
  ...
  const handleFlagResource = async () => {
    ...
    if (!trimmedReason) {
      toast.warn("Please specify the nature of the breach.");
      return;
    }
    if (trimmedReason.length > 500) {
      toast.warn("Security report exceeds maximum character threshold.");
      return;
    }

    setSubmittingFlag(true);
    try {
      await api.post(`/resources/${id}/flag`, { reason: trimmedReason });
    ...
    return (
      <div className="flex flex-col items-center justify-center py-40 px-6 text-center bg-[#FAF8F4] dark:bg-[#0C0603] min-h-screen">
        <div className="mb-8 p-10 bg-white dark:bg-white/5 rounded-[3rem] shadow-2xl">
          ...
          <h2 className="text-3xl font-black text-[#5A270F] dark:text-white uppercase tracking-tighter italic mb-4">
            Registry Fault
          </h2>
          <p className="max-w-md text-[#92664A] dark:text-[#EEB38C]/60 text-sm font-medium italic">
            {error || "Asset not found."}
          </p>
        </div>
        <button
          onClick={() => navigate(-1)}
          className="px-10 py-4 bg-[#5A270F] text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-[#DF8142] transition-all"
        >
          Return to Library
        </button>
      </div>
    ...
    <div className="min-h-screen bg-[#FAF8F4] dark:bg-[#0C0603] selection:bg-[#DF8142]/30 transition-colors duration-700">
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-8 space-y-12">
              ...
              <div className="relative bg-white dark:bg-[#1A0B04] rounded-[3rem] border-2 border-[#92664A]/10 dark:border-white/5 overflow-hidden shadow-2xl">
                ...
                <div className="flex flex-col lg:grid lg:grid-cols-12 relative z-10">
                  ...
                  <div className="lg:col-span-4 p-8 bg-[#EEB38C]/5 dark:bg-black/20 flex flex-col items-center justify-center text-center space-y-6 border-r-2 border-[#92664A]/10">
                    ...
                    <div>
                      <h4 className="text-[9px] font-black uppercase tracking-[0.4em] text-[#92664A] mb-1.5">
                  ...
                  <div className="lg:col-span-8 p-8 lg:p-10 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-6 pt-6 border-t border-[#92664A]/10">
                      <div className="space-y-1">
                        <p className="text-[8px] font-black text-[#92664A] uppercase tracking-[0.4em]">
                          Primary Architect
                        </p>
                        <p className="text-xs font-black text-[#5A270F] dark:text-white uppercase italic">
                          {resource.author}
                        </p>
                      </div>
            ...
            <section className="bg-white dark:bg-[#1A0B04] rounded-[2.5rem] p-8 lg:p-10 border-2 border-[#92664A]/10 shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col md:grid md:grid-cols-12 gap-10">
                <div className="md:col-span-4 space-y-5">
                  <div className="space-y-3">
                    ...
                    <div className="p-6 rounded-[2rem] bg-[#5A270F] text-white text-center shadow-xl relative group/rating overflow-hidden">
                      ...
                      <span className="text-5xl font-black italic relative z-10 font-space-grotesk">
                        {averageRating.toFixed(1)}
                      </span>
                      <p className="text-[9px] font-black uppercase tracking-[0.5em] text-[#EEB38C] mt-1.5 relative z-10">
                        Sync_Average
                      </p>
                    </div>
                  </div>
                ...
                <div className="md:col-span-8 flex flex-col justify-center space-y-6">
                  <div className="space-y-3">
                    <p className="text-[9px] font-black text-[#92664A] uppercase tracking-[0.3em]">
                    ...
                    <div className="flex items-center gap-3">
                      {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        >
                          <Star
                            className={`h-8 w-8 ${(hoverRating || pendingRating || userRating || Math.round(averageRating)) >= star ? "text-[#DF8142] fill-[#DF8142]" : "text-[#EEB38C]/20"}`}
                          />
                        </button>
                      ))}
                    </div>
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[9px] font-black text-[#92664A] uppercase tracking-widest italic">
              ...
              <div className="grid gap-5">
                {comments.map((comment) => (
                  <div
                    key={comment.id}
                    className="relative group/comment animate-in fade-in slide-in-from-bottom-5 duration-700"
                  >
                    <div className="bg-white dark:bg-[#1A0B04] p-6 rounded-[2rem] border border-[#92664A]/10 shadow-lg relative z-10 group-hover/comment:border-[#DF8142]/40 transition-colors">
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="space-y-1.5 flex-1">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black text-[#5A270F] dark:text-white uppercase tracking-widest italic">
                              Node_{comment.user.id.slice(0, 4)}
                            </span>
                            <span className="text-[8px] font-black text-[#92664A]/40 uppercase tracking-[0.2em]">
          ...
          <aside className="lg:col-span-4 space-y-8 lg:sticky lg:top-8">
            ...
            <div className="bg-[#5A270F] p-8 rounded-[2.5rem] text-white shadow-xl relative overflow-hidden group/ops">
              ...
              <div className="relative z-10 space-y-8">
                <div className="space-y-1.5">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#EEB38C]">
                    Operations
                  </h4>
                ...
                {isAuthorizedManager && (
                  <div className="pt-6 border-t border-white/10 space-y-3">
                    ...
                    >
                      ...
                      {resource.is_public ? "Global" : "Internal"}
                    </button>
                    {resource.status === "archived" ? (
                      <button
                        onClick={handleRestore}
                        title="Restore Node Sync"
                        className="w-full h-12 bg-emerald-500/10 border border-emerald-500/30 rounded-lg flex items-center justify-center gap-3 text-emerald-400 text-[9px] font-black uppercase tracking-[0.3em] hover:bg-emerald-500/20 transition-all"
                      >
                      ...
                      <button
                        ...
                        title="Initiate Secure Sequestration"
                        className="w-full h-12 bg-red-500/10 border border-red-500/30 rounded-lg flex items-center justify-center gap-3 text-red-400 text-[9px] font-black uppercase tracking-[0.3em] hover:bg-red-500/20 transition-all"
                      >
                        <Trash2 className="h-4 w-4" /> Archive Resource
                      </button>
                    )}
                  </div>
                )}
                ...
                <div className="flex items-center justify-between p-4 bg-black/40 rounded-2xl border border-white/5">
                  <div className="space-y-0.5">
                    <p className="text-[7px] font-black text-[#EEB38C]/40 uppercase tracking-widest">
                      Registry_Handle
                    </p>
                    <p className="text-lg font-black text-[#EEB38C] font-mono tracking-tighter">
            ...
            <div className="bg-white dark:bg-[#1A0B04] p-8 rounded-[2.5rem] border-2 border-[#92664A]/10 shadow-xl space-y-8 relative overflow-hidden group/synapse">
              <div className="flex items-center justify-between">
                <h5 className="text-[10px] font-black uppercase tracking-[0.5em] text-[#5A270F] dark:text-[#EEB38C]">
                  Synaptic_Nodes
                </h5>
              ...
              <div className="grid gap-4">
                {recentResources.map((recent) => (
                  <Link
                    ...
                    to={
                      isNested
                        ? `${basePath}/resources/${recent.id}`
                        : `/resources/${recent.id}`
                    }
                    className="flex items-center gap-3 group/node transition-all"
                  >
                    ...
                    <div className="overflow-hidden">
                      <p className="text-[11px] font-black text-[#5A270F] dark:text-white uppercase truncate tracking-tighter italic">
                        {recent.title}
                      </p>
                      <p className="text-[8px] font-black text-[#92664A]/60 uppercase tracking-widest mt-0.5">
                        Arch. {recent.author}
                      </p>
                    </div>
                  </Link>
            ...
            <div className="bg-[#DF8142] p-6 rounded-[2rem] text-white flex items-center justify-between">
              <div className="space-y-0.5">
                <h5 className="text-[9px] font-black uppercase tracking-[0.4em]">
                  Arch_Nexus_V2
                </h5>
                <p className="text-[7px] font-bold opacity-60 uppercase tracking-widest">
                  Restricted Internal Node
                </p>
              </div>
      ...
      {showFlagModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          <div
            className="absolute inset-0 bg-[#5A270F]/90 backdrop-blur-xl animate-in fade-in duration-500"
          ...
          <div className="relative w-full max-w-lg bg-white dark:bg-[#1A0B04] rounded-[3rem] p-10 space-y-8 animate-in zoom-in-95 shadow-[0_50px_100px_rgba(0,0,0,0.5)] border-2 border-[#DF8142]/30">
            <div className="flex items-center justify-between">
              <div className="space-y-2">
                ...
                <p className="text-[10px] font-black text-[#DF8142] uppercase tracking-[0.4em]">
                  Security integrity reporting nexus
                </p>
              </div>
              <button
                onClick={() => setShowFlagModal(false)}
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-[9px] font-black uppercase tracking-[0.3em] text-[#92664A] ml-2">
