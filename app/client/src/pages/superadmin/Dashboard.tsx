    ...
    >
      ...
      {!isSettingsPage && (
        <button
          ...
          className="lg:hidden fixed bottom-8 right-8 z-[60] h-16 w-16 bg-[#5A270F] text-white rounded-full shadow-2xl flex items-center justify-center active:scale-90 transition-transform border-4 border-white/20"
      ...
      {/* Super Sidebar (Unified Premium Style) */}
      {!isSettingsPage && (
        <>
          {/* Overlay for mobile */}
          {isSidebarOpen && (
          ...
          >
            <div className="bg-gradient-to-b from-[#1A0B04] via-[#5A270F] to-black rounded-[2.5rem] p-8 text-white relative overflow-hidden flex flex-col gap-8 shadow-2xl shadow-black/30 dark:shadow-none border border-white/5 h-full lg:h-auto min-h-[85vh] transition-colors duration-500">
              <div className="absolute inset-0 opacity-[0.03] pointer-events-none architectural-dot-grid" />
              <div className="absolute top-0 right-0 w-48 h-48 bg-[#DF8142]/10 blur-[80px] -translate-y-1/2 translate-x-1/2" />
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all group ${
                          isActive
                            ? "bg-white text-[#5A270F] shadow-xl shadow-black/40 scale-[1.02]"
              ...
              <div className="space-y-4 relative z-10">
                ...
                <nav className="space-y-2">
                  {[
                    { to: "maintenance", label: "Engine Room", icon: Settings },
                    { to: "backup", label: "Vault Control", icon: Globe },
                    { to: "external", label: "Outbound Nodes", icon: ExternalLink },
                  ].map((item) => (
                    <NavLink
                      key={item.to}
                      to={item.to}
            ...
            <div className="bg-gradient-to-br from-black to-[#1A0B04] rounded-[2.5rem] p-8 text-white relative overflow-hidden flex flex-col justify-end min-h-[180px] shadow-2xl shadow-black/40 hidden lg:flex border border-white/5">
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#DF8142]/10 blur-3xl" />
              <ShieldAlert className="h-4 w-4 mb-4 relative z-10 text-[#DF8142]" />
              <p className="text-[10px] font-black uppercase tracking-[0.3em] relative z-10 text-white/40">
                Access Level
              </p>
              <h3 className="text-xl font-black relative z-10 uppercase tracking-tighter">
                Super Admin
              </h3>
            </div>
          </aside>
