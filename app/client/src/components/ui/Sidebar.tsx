...
export default function Sidebar({ isOpen, setOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white border-r p-4 space-y-2 transform ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:w-56 z-20`}
      >
        ...
        {isAdmin && (
          <>
            ...
            <Link
              onClick={closeSidebar}
              className={`block px-3 py-2 rounded ${active("/admin/flags")}`}
              to="/admin/flags"
            >
              Flags
            </Link>
